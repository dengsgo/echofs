...
一个轻量级的 Rust 文件服务器，编译为单个可执行文件。

通过浏览器浏览目录、预览媒体文件、复制分享链接。支持完整读写 WebDAV，可在电脑、手机、平板上将共享目录挂载为网络驱动器，随时随地跨设备访问、管理文件。

## 功能特性

- **单文件部署** — 极小体积，无运行时依赖，即开即用，极速运行
- **多平台支持** — 提供 Linux（AMD64/ARM64）、macOS（Intel/Apple Silicon）、Windows（AMD64）的预编译二进制；跨操作系统、跨架构体验一致
- **适用场景** — 文件共享：团队、朋友间快速分发文件；PC / 手机 / 平板互联：跨设备无缝访问；轻量 NAS：文件管理与大片在线观看，畅享影音
- **目录浏览** — 现代化 Web UI，支持面包屑导航
- **媒体预览** — 视频、音频、图片在浏览器内直接播放预览（图片支持画廊模式，滑动/键盘导航）
- **链接分享** — 一键复制文件链接和二维码，可在 VLC、mpv 等外部播放器中使用
- **Range 请求** — 视频拖动进度条，支持断点续传
- **文件排序** — 按名称、大小、创建/修改时间排列
- **多主题切换** — Classic（经典）、Liquid Glass（iOS 26 液态玻璃风）、Cartoon（卡通漫画风），均支持明暗色模式
- **响应式布局** — 移动端卡片布局、桌面端表格布局
- **网页端文件管理** — 直接在浏览器中上传、重命名、删除文件（基于 WebDAV 实现）
...
130 测试覆盖各模块单元测试和完整 HTTP 路由集成测试。
